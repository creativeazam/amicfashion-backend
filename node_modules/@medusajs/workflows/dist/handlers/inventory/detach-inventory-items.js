...
async function detachInventoryItems({ container, context, data, }) {
    ...
    await Promise.all(data.inventoryItems.map(async ({ variant, inventoryItem }) => {
        return await productVariantInventoryService.detachInventoryItem(inventoryItem.id, variant.id);
    }));
}
