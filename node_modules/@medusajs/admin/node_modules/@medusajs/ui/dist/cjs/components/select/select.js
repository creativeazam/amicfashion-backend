...
exports.Select = exports.useSelectContext = exports.SelectContext = void 0;
const tslib_1 = require("tslib");
const icons_1 = require("@medusajs/icons");
const DropdownPrimitive = tslib_1.__importStar(require("@radix-ui/react-dropdown-menu"));
...
const useSelectContext = () => {
    const context = React.useContext(exports.SelectContext);
    if (context === null)
...
const Root = ({ children, multi = false, onChange, search = false, onSearch = () => { }, onScrollToBottom = () => { }, ...props }) => {
    ...
    const removeItem = (item) => setItems((items) => items.filter((existingItem) => !(0, lodash_1.isEqual)(existingItem, item)));
    ...
    const selectReturn = (0, downshift_1.useSelect)(selectProps);
    return (React.createElement(DropdownPrimitive.Root, { ...props, modal: false, onOpenChange: onOpenChange },
        React.createElement(exports.SelectContext.Provider, { value: {
                multi,
                selectedItems,
                clearSelectedItems,
                selectAll,
                ...
                addItem,
                removeItem,
                onScrollToBottom: (0, lodash_1.throttle)(onScrollToBottom, 500),
                ...selectReturn,
            } }, children)));
};
Root.displayName = "Select.Root";
const Trigger = React.forwardRef(({ className, children, size = "regular", disabled, ...props }, ref) => {
    const { getToggleButtonProps, selectedItem, search } = (0, exports.useSelectContext)();
    const { ref: toggleButtonRef, ...toggleButtonProps } = getToggleButtonProps({
        disabled,
        ref,
    });
    return (React.createElement(DropdownPrimitive.Trigger, { ref: toggleButtonRef, asChild: true, className: (0, clx_1.clx)("bg-ui-bg-subtle border-ui-border-loud-muted text-ui-fg-base shadow-buttons-secondary txt-compact-medium group/trigger relative z-10 flex h-10 w-full cursor-pointer items-center rounded-md border px-3 py-[9px] transition-all", "hover:bg-ui-bg-subtle-hover", {
        ...
        React.createElement("div", null,
            search && (React.createElement("div", { className: "text-ui-fg-muted absolute left-3 flex h-5 w-5 items-center justify-center" }, React.createElement(icons_1.MagnifyingGlassMini, null))),
            children)));
});
Trigger.displayName = "Select.Trigger";
const TriggerIcon = ({ className, children, ...props }) => (React.createElement("div", { className: (0, clx_1.clx)("text-ui-fg-muted group-data-[disabled]/trigger:text-ui-fg-disabled z-10 ml-auto", className), ...props }, children ? children : React.createElement(icons_1.ChevronUpDown, null)));
TriggerIcon.displayName = "Select.TriggerIcon";
const Value = ({ children, placeholder = "", value }) => {
    const { selectedItem, multi, selectedItems, clearSelectedItems, search, isOpen, onSearch, } = (0, exports.useSelectContext)();
    if (search && isOpen)
    ...
    if (multi && selectedItems)
        return (React.createElement("div", { className: "text-ui-fg-base" },
            React.createElement(badge_1.Badge, { className: "relative z-20 mr-1", onClick: () => clearSelectedItems() },
...
const Content = React.forwardRef(({ children, className, ...props }, ref) => {
    ...
    const { getMenuProps, onScrollToBottom, isOpen } = (0, exports.useSelectContext)();
...
const Item = React.forwardRef(({ className, children, item, ...props }, ref) => {
    ...
    return (React.createElement(DropdownPrimitive.Item, { ref: itemRef, className: (0, clx_1.clx)("relative flex w-full cursor-default select-none items-center rounded-md py-2 pl-10 pr-3 text-sm", "hover:bg-ui-bg-base-hover", "focus:bg-ui-bg-base-hover focus:outline-none", "data-[disabled]:text-ui-fg-disabled data-[disabled]:cursor-not-allowed data-[disabled]:hover:bg-transparent", { "font-medium": isSelected }, className), ...props, ...itemProps, onClick: (e) => {
            if (multi)
                e.preventDefault();
            if (selectItem)
                selectItem(item);
        ...
        isSelected && (React.createElement("div", { className: "absolute left-3 flex h-5 w-5 items-center justify-center" }, multi ? React.createElement(icons_1.CheckMini, null) : React.createElement(icons_1.EllipseMiniSolid, null))),
        children));
});
Item.displayName = "Select.Item";
const Separator = React.forwardRef(({ className, ...props }, ref) => (React.createElement(DropdownPrimitive.Separator, { ref: ref, className: (0, clx_1.clx)("bg-ui-border-base -mx-1 my-1 h-px", className), ...props })));
Separator.displayName = "Select.Separator";
const SelectAll = ({ children, className, ...props }) => {
    const { multi, selectAll, allSelected } = (0, exports.useSelectContext)();
    if (!multi) {
        console.error('Select.SelectAll can only be used within a Select with multi="true"');
        return null;
    }
    return (React.createElement("div", { className: (0, clx_1.clx)("relative flex w-full cursor-default select-none items-center rounded-md py-2 pl-10 pr-3 text-sm", "hover:bg-ui-bg-base-hover", "focus:outline-none", className), onClick: (e) => {
            e.preventDefault();
            selectAll();
        }, 
...
const SearchInput = React.forwardRef(({ className, ...props }, ref) => {
    const innerRef = React.useRef(null);
    React.useImperativeHandle(ref, () => innerRef.current);
    return (React.createElement("input", { ref: innerRef, className: (0, clx_1.clx)("caret-ui-fg-base txt-compact-medium text-ui-fg-base max-w-[90%] bg-transparent focus:outline-none", className), placeholder: "Find something", 
...
const Search = React.forwardRef(({ icon, ...props }, ref) => {
    return (React.createElement("div", { className: "txt-compact-medium relative flex items-center px-3 py-2 pl-10" },
        React.createElement("div", { className: "text-ui-fg-muted absolute left-3 flex h-5 w-5 items-center justify-center" }, icon ? icon : React.createElement(icons_1.MagnifyingGlassMini, null)),
        React.createElement(SearchInput, { ...props, ref: ref })));
});
Search.displayName = "Select.Search";
const Label = React.forwardRef(({ className, ...props }, ref) => (React.createElement(DropdownPrimitive.Label, { ref: ref, className: (0, clx_1.clx)("text-ui-fg-subtle txt-compact-xsmall-plus px-2 py-1.5", className), ...props })));
